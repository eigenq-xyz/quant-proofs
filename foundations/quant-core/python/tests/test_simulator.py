...
class TestSimulateGBM:
    ...
    def test_reproducible_with_seed(self) -> None:
        p1 = simulate_gbm(
            S0=100.0, mu=0.05, sigma=0.20, T=1.0, n_steps=52, seed=42
        )
        p2 = simulate_gbm(
            S0=100.0, mu=0.05, sigma=0.20, T=1.0, n_steps=52, seed=42
        )
    ...
    def test_different_seeds_differ(self) -> None:
        p1 = simulate_gbm(
            S0=100.0, mu=0.05, sigma=0.20, T=1.0, n_steps=52, seed=1
        )
        p2 = simulate_gbm(
            S0=100.0, mu=0.05, sigma=0.20, T=1.0, n_steps=52, seed=2
        )
        assert p1.prices != p2.prices

    def test_all_prices_positive(self) -> None:
        path = simulate_gbm(
            S0=100.0, mu=0.05, sigma=0.20, T=1.0, n_steps=252, seed=0
        )

...
the formally verified Lean 4 PGD solver in `foundations/optimization-proofs/`.
...
`pgd_solve` binary from `foundations/optimization-proofs/` in a persistent subprocess so
...
```bash
# Install dev dependencies (from foundations/portfolio-proofs/)
uv sync --extra dev
...
- `lean_pgd_ffi.py`: Cython FFI wrapper, dispatches to FFI-flat or FFI-boxed
  ...
  Build: `cd foundations/optimization-proofs && python ffi/setup_ffi.py build_ext --inplace`

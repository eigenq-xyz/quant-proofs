"""KKT analytical derivation of the global minimum for the step-divergence scenario.

For a strictly convex QP the KKT conditions are both necessary and sufficient
(Boyd and Vandenberghe 2004, §5.5.3). The derivation uses the post-shock
shrunk covariance (Sigma_shock) and mean returns (mu_shock):

1. Identify the support: Utils has the highest mean return (-0.726%/day, least
   negative in the crash) as the long leg; Enrgy has the lowest (-1.906%/day)
   as the short leg.
2. Solve the 2x2 linear system when both budget and leverage constraints are
   simultaneously tight to get exact weights:
       w_Utils + w_Enrgy = 1   (budget tight)
       w_Utils - w_Enrgy = 1.5 (leverage tight, w_Utils > 0, w_Enrgy < 0)
       => w_Utils = 1.25, w_Enrgy = -0.25
...
def derive(p: ProblemData) -> tuple[SolverResult, KKTCertificate]:
    ...
    ----------
    p:
        ProblemData; uses p.Sigma_shock, p.mu_shock, p.leverage_cap, p.industries.

    Returns
    -------
    (SolverResult, KKTCertificate)
    """
    L = p.leverage_cap

    # Step 1: support from return spread (using post-shock mu_shock)
    idx_long = int(
        np.argmax(p.mu_shock)
    )  # Utils: highest (least negative) return
    idx_short = int(np.argmin(p.mu_shock))  # Enrgy: lowest return
    ...
    # grad f(w*) + lam * 1 + nu * sign(w*) = 0
    # For active positions: Sigma_shock @ w* - mu_shock + lam * e_k + nu * sign(w_k) = 0
    r = p.Sigma_shock @ w_star - p.mu_shock
    lam = -(r[idx_long] + r[idx_short]) / 2
    nu = (r[idx_short] - r[idx_long]) / 2

    # Dual feasibility for zero-weight industries: |r_k + lam| <= nu
...
def print_certificate(
    ...
    print(
        ...
        f"w = {cert.w_long:+.4f}  "
        f"(mu = {p.mu_shock[cert.idx_long] * 100:+.4f}%/day, highest)"
    )
    print(
        f"Short leg : {p.industries[cert.idx_short]:6s}  "
        f"w = {cert.w_short:+.4f}  "
        f"(mu = {p.mu_shock[cert.idx_short] * 100:+.4f}%/day, lowest)"
    )
    print()
    ...
    print(f"  lambda (budget)   = {cert.lam:.6f}")
    print(
        f"  nu (leverage)     = {cert.nu:.6f}  (>= 0) {'✓' if cert.nu >= 0 else '✗'}"
    )
    print()
    print("Dual feasibility — all zero-weight industries must satisfy")
    print(f"|r_k + lambda| <= nu = {cert.nu:.6f}:")
    print(
        f"  {'Industry':>8}  {'|r_k + lambda|':>14}  {'Slack':>10}  {'OK?':>4}"
    )
    inactive = [
    ...
    for k, slack in zip(inactive, cert.slack_margins, strict=True):
        ...
        print(f"  {p.industries[k]:>8}  {val:14.6f}  {slack:10.6f}  {flag}")
    print()

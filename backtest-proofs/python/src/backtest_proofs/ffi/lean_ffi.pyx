...
Exposes @[export hedge_*] functions from BacktestProofs.Accounting.
...
cdef extern from *:
    ...
    extern lean_object* initialize_backtest_x2dproofs_BacktestProofs_Accounting(
        uint8_t builtin);

    // FFI exports from BacktestProofs.Accounting
    ...
    lean_object* initialize_backtest_x2dproofs_BacktestProofs_Accounting(
        uint8_t builtin)
...
def initialize_lean():
    """Initialise Lean runtime and the BacktestProofs.Accounting module.

    Must be called exactly once before any other FFI function.
    """
    lean_initialize_runtime_module()
    cdef lean_object* res = \
        initialize_backtest_x2dproofs_BacktestProofs_Accounting(1)
    if lean_io_result_is_error(res):
        lean_dec(res)
        raise RuntimeError("Failed to initialise Lean BacktestProofs.Accounting module")
    lean_dec(res)

...
def process(
    ...
    from mortgage_proofs.orchestrator.runner import run_sync
    from mortgage_proofs.record.io import write
    ...
    try:
        ...
        typer.echo(
            "Skipping verification (build the Lean binary with `make lean-build`)."
        )
        return
...
def design(
    ...
    from mortgage_proofs.record.design_session_io import serialize
    ...
    if goal:
        ...
        from mortgage_proofs.domain.enums import GoalPriority
    ...
    record = run_design_sync(
        applicant_situation, mortgage_goal, max_iterations=max_iter
    )
...
def _print_design_outcome(record: object) -> None:
    ...
    from mortgage_proofs.record.models import DesignSessionRecord
...
def schema_dump(
    ...
    from mortgage_proofs.record.schema import get_json_schema

...
from mortgage_proofs.lean_bridge.config import (
    get_binary_path,
    get_timeout_seconds,
)
...
from mortgage_proofs.record.io import serialize
...
def verify_file(path: Path) -> VerificationResult:
    ...
    binary = get_binary_path()
    if not binary.exists():
        raise LeanBinaryNotFoundError(f"Lean verifier binary not found at {binary}.")

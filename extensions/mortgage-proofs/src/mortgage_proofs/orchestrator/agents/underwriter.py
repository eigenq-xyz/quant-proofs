...
def _format_prior_decisions(state: GraphState) -> str:
    ...
    for d in state["decisions"]:
        lines.append(
            f"  [{d.agent_name}] {d.outcome.value} (confidence: {d.confidence_score})"
        )
...
def underwriter_node(state: GraphState) -> dict:  # type: ignore[type-arg]
    ...
    response = cast(
        ...
        llm.invoke(
            [
                ...
                HumanMessage(
                    content=UNDERWRITER_HUMAN.format(
    ...
    return {
        ...
        "escalation_required": (response.outcome.value == "ESCALATE_TO_UNDERWRITER"),
        "escalation_reason": response.escalation_reason,
    }

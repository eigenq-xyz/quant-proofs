...
from mortgage_proofs.domain.enums import RoutingOutcome, SessionOutcome
...
from mortgage_proofs.orchestrator.agents.package_designer import (
    package_designer_node,
)
from mortgage_proofs.orchestrator.agents.package_reviewer import (
    package_reviewer_node,
)
from mortgage_proofs.orchestrator.agents.risk import risk_node
from mortgage_proofs.orchestrator.agents.router import route_after_analysis
from mortgage_proofs.orchestrator.agents.underwriter import underwriter_node
from mortgage_proofs.orchestrator.config import get_llm
from mortgage_proofs.orchestrator.prompts import (
    ESCALATION_SUMMARY_HUMAN,
    ESCALATION_SUMMARY_SYSTEM,
)
from mortgage_proofs.orchestrator.state import DesignGraphState, GraphState
from mortgage_proofs.record.models import DecisionRecord, DesignSessionRecord
...
def _assemble_escalated(state: DesignGraphState) -> dict:  # type: ignore[type-arg]
    ...
    monthly_income = situation.monthly_income_usd
    existing_dti = (
        situation.debt_obligations_monthly_usd / monthly_income if monthly_income else 0
    )
    ...
    summary_response = llm.invoke(
        [
            ...
            HumanMessage(
                content=ESCALATION_SUMMARY_HUMAN.format(

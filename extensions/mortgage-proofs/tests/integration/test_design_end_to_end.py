...
def test_escalated_after_max_iterations(
    ...
    escalation_msg.content = (
        "DTI violations persist across 2 iterations. Recommend specialist review."
    )
    ...
    with (
        ...
        patch(_REVIEWER_MODULE, return_value=_reviewer_mock()),
        patch(
            _VERIFY_MODULE, return_value=_lean_violation_result("dtiCapConventional")
        ),

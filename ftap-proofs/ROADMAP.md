...
- [x] **M1.1** Define `FinancialMarket`: finite `Ω`, time horizon `T`, filtration
      `(ℱ_t)_{t=0..T}`, risky asset prices `S : Fin n → Fin (T+1) → Ω → ℝ`,
      numeraire `B : Fin (T+1) → ℝ`
- [x] **M1.2** Measurability typeclass: `S i t` is `ℱ_t`-measurable for all `i, t`
- [x] **M1.3** Define discounted prices `S̃ i t ω := S i t ω / B t`
- [x] **M1.4** Lemma: `B t > 0` for all `t` (positivity of numeraire)
- [x] **M1.5** Lemma: `S̃ i t` is `ℱ_t`-measurable (inherited from `S`)
...
- [x] **S2.1** Define `TradingStrategy`: `θ : Fin n → Fin (T+1) → Ω → ℝ`
      where `θ i t` is `ℱ_{t-1}`-measurable (predictable)
- [x] **S2.2** Define value process `V t θ ω := ∑ i, θ i t ω * S i t ω`
- [x] **S2.3** Define discounted value process `Ṽ t θ := V t θ / B t`
- [x] **S2.4** Define `selfFinancing θ`: no cash injections between rebalances
      (`V t θ = V (t-1) θ + ∑ i, θ i t * (S i t - S i (t-1))`)
- [x] **S2.5** Define discounted gains process
      `G t θ := ∑_{s=1}^{t} ∑ i, θ i s * (S̃ i s - S̃ i (s-1))`
- [x] **S2.6** Lemma: `selfFinancing θ ↔ Ṽ t θ = Ṽ 0 θ + G t θ` for all `t`
...
- [x] **A3.1** Define `ArbitrageOpportunity θ`:
      `selfFinancing θ ∧ V 0 θ = 0 ∧ V T θ ≥ 0 a.s. ∧ P {ω | V T θ ω > 0} > 0`
- [x] **A3.2** Define `NoArbitrage m`: `¬ ∃ θ, ArbitrageOpportunity m θ`
- [x] **A3.3** Define attainable payoff set
      `K m := {G T θ | θ self-financing, V 0 θ = 0}`
- [x] **A3.4** Lemma: `K m` is a linear subspace of `Ω → ℝ`
- [x] **A3.5** Lemma: `NoArbitrage m ↔ K m ∩ {f | f ≥ 0 a.s., f ≠ 0 a.s.} = ∅`
...
- [x] **Q4.1** Define `EquivalentMeasure P Q`: `∀ A, P A = 0 ↔ Q A = 0`
- [x] **Q4.2** Define `IsMartingaleMeasure m Q`:
      `∀ i t, E^Q [S̃ i (t+1) | ℱ_t] = S̃ i t`
- [x] **Q4.3** Define `EquivalentMartingaleMeasure m Q`:
      `EquivalentMeasure P Q ∧ IsMartingaleMeasure m Q`
- [x] **Q4.4** Lemma: under any EMM `Q`, `Ṽ t θ` is a `Q`-martingale for
      any self-financing `θ` — the key property linking the two halves
- [x] **Q4.5** Lemma (risk-neutral pricing): `V 0 θ = E^Q [V T θ / B T]`
...
- [x] **T5.1** Theorem `emm_implies_no_arbitrage`:
...
- [x] **T5.2** Key lemma `separating_functional`:
      `NoArbitrage m → ∃ φ : (Ω → ℝ) →ₗ[ℝ] ℝ, StrictlyPositive φ ∧ ∀ f ∈ K m, φ f = 0`
      (Farkas' lemma / separating hyperplane in `ℝ^|Ω|`)
- [x] **T5.3** Lemma `state_prices_to_emm`: construct `Q` from `φ` and verify
      `EquivalentMartingaleMeasure m Q`
- [x] **T5.4** Theorem `no_arbitrage_implies_emm`:

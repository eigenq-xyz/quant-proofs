...
This non-anticipation property is already formalized, sorry-free, in
`research-pipeline/lean/ResearchPipeline/Measurability.lean`, which proves that a signal
built from prices observed up to `t` is adapted to the natural filtration generated by the
price process. The signal layer of this study therefore needs no new proof: the verification
edge carries over directly. The statistical and P&L layers below are empirical and unverified.
...
falls to 0.99, and a conservative combination (5 bps per rebalance plus a 3% half-spread) lands
the OOS Sharpe at **0.73**, squarely inside the literature range. At very wide spreads
(10 bps plus a 5% half-spread) the premium is fully consumed and the strategy turns negative.
